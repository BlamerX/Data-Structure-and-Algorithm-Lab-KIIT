...
void getData93(int a[size93][size93], int row, int column)
{
    for (int i = 0; i < row; i++)
    {
...
void create93(int a[size93][size93], int row, int column, int b[size93][3])
{
    int k = 0;
    b[0][0] = row;
    b[0][1] = column;
...
int q3()
{
    ...
    int arr[row][column], arr2[size93][3];

    getData93(arr, row, column);
    create93(arr, row, column, arr2);
    display93(arr2);
